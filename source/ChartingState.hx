...
/*
Here's a little reference to the code in "Shaggy X Matt" mod for some UI Things UwU
...
Sorry I'm not good at coding so...
*/
...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var typeNames:Array<String> = ['Normal', 'Death', 'Stun'];
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			else if (wname == 'note_type')
			{
				...
				trace('wname note type is ' + noteType);
				typeNameTxt.text = typeNames[noteType] + ' notes';
				//updateGrid();
			}
		}
	...
	private function addNote(?n:Note):Void
	{
		...
		if (FlxG.keys.pressed.ALT)
			noteType = 2;
		if (FlxG.mouse.justPressed && !FlxG.mouse.overlaps(curRenderedNotes))
			noteType = 3;*/
		if (n != null){
			_song.notes[curSection].sectionNotes.push([n.strumTime, n.noteData, n.sustainLength, n.noteType]);
			trace('n.noteType is ' + n.noteType);
		}
		else{
			_song.notes[curSection].sectionNotes.push([noteStrum, noteData, noteSus, noteType]);
			trace('noteType is ' + noteType);
		}
